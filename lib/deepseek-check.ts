...
const CHECK_SYSTEM_PROMPT = `Ты аналитик по проверке российских мобильных номеров перед исходящим обзвоном.
На входе — сниппеты из поисковика и сводка других источников.
Оцени, есть ли признаки спама, нежелательного звонка или мошенничества.

Отвечай ТОЛЬКО валидным JSON без markdown и пояснений:
{"verdict":"OK"|"CAUTION"|"REJECT","confidence":0.0,"isUnwanted":false,"isNegative":false,"summary":"кратко по-русски","signals":["..."]}

Правила:
- REJECT только при явных признаках в данных: нежелательный, спам, мошенник, отрицательная оценка
- CAUTION при слабых/неоднозначных сигналах или мало данных
- OK если негатива нет
...
function parseVerdict(value: unknown): Verdict {
  if (value === 'REJECT' || value === 'CAUTION' || value === 'OK') {
    return value;
  }
  return 'CAUTION';
}
